...
def load_gt(nusc: NuScenes, eval_split: str, box_cls, verbose: bool = False, custom_subset=None):
    """
    ...
    if custom_subset is not None:
        print('[WARNING] Custom subset is used for evaluation. Sample length: {}'.format(len(custom_subset)))
        sample_tokens_all = [s['token'] for s in nusc.sample if s['token'] in custom_subset]
    else:
...
class NuScenesEval_custom(NuScenesEval):
    ...
    def __init__(self,
        ...
        self.pred_boxes, self.meta = load_prediction(self.result_path, self.cfg.max_boxes_per_sample, DetectionBox,
                                                     verbose=verbose)
        self.gt_boxes = load_gt(self.nusc, self.eval_set, DetectionBox_modified, 
                                verbose=verbose, custom_subset=custom_subset)
        print(len(custom_subset), self.nusc)
