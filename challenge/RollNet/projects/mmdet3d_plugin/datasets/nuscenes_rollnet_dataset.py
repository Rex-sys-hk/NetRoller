...
class v1CustomDetectionConfig:
    ...
    def __init__(self,
                 class_range: Dict[str, int], # TODO: for backward compatibility
...
# TODO
from .lm_dataset import FinetuneDataset, transform_train
@DATASETS.register_module()
class RollNetCustomNuScenesDataset(NuScenesDataset):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.lm_dataset = FinetuneDataset(transform_train, 
                                        max_words, 
                                        tokenizer_path, 
                                        text_desc_types,
                                        text_latest_frame)
        # indices = torch.randperm(len(self.data_infos))[:int(len(self.data_infos) * used_data_ratio)]
        # self.data_infos = [self.data_infos[i] for i in indices]
        self.data_infos = self.data_infos[:int(len(self.data_infos) * used_data_ratio)]
        self.sample_tokens = [info['token'] for info in self.data_infos]
        print(f'Using {len(self.data_infos)} samples for training.')
    ...
    def prepare_train_data(self, index):
        ...
        example = self.union2one(data_queue)
        example.update(input_dict['texts'])
        return example
    ...
    def get_data_info(self, index):
        ...
        # if info['text_labels']:
        input_dict['texts']=self.lm_dataset.get_text_label(info['text_labels'],info['token'])
        input_dict['texts_time_offset'] = \
            (input_dict['texts']['timestamp']-info['timestamp'])/1e6
        # print(input_dict.keys()) # TODO
        return input_dict
    ...
    def _format_bbox(self, results, jsonfile_prefix=None, score_thresh=0.2):
        ...
        for sample_id, det in enumerate(mmcv.track_iter_progress(results)):
            ...
            try:
                textual_results[sample_token] = {'prompts':det['prompts'], 'decoded':det['decoded']}
            except:
                pass
    ...
    def anti_format_bbox(self, results, jsonfile_prefix=None):
        """Convert the results to the standard format.

        Args:
            results (dict): Testing results of the dataset saved in file.
            jsonfile_prefix (str): The prefix of the output jsonfile.
                You can specify the output directory/filename
                by modifying the jsonfile_prefix. Default: None.
        Returns:
            original results: (list[dict]).
        """
        pass
    ...
    def format_results(self, results, jsonfile_prefix=None):
        ...
        """
        if isinstance(results, dict) and 'bbox_results' in results.keys():
    ...
    def _evaluate_single(self,
        ...
        self.nusc_eval = NuScenesEval_custom(
            ...
            custom_subset=self.sample_tokens,
        )
        ...
        map_annotations = [m for m in map_annotations if m['sample_token'] in self.sample_tokens]
    ...
    def evaluate(self,
        ...
        for k, v in result_dict.items():
            print(f'{k}: {v}')
        # save results to prediction_summary.json
        mmcv.mkdir_or_exist(jsonfile_prefix)
        with open(os.path.join(jsonfile_prefix,'instance_wise_results.pkl'), "wb") as pf:
            pickle.dump(results, pf)
        with open(os.path.join(jsonfile_prefix,'prediction_summary.json'), "w") as jf:
            for k, v in result_dict.items():
        ...
        # save results to planning_summary.json
        with open(os.path.join(jsonfile_prefix,'planning_summary.json'), "w") as jf:
            for k, v in metric_dict.items():
                metric_dict[k] = float(v)
            json.dump(metric_dict, jf, indent=4)
        ...
        with open(os.path.join(jsonfile_prefix,'results_dict.pkl'), "wb") as pf:
