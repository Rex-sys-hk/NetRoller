...
class SparseAttention(Module):
    ...
    def forward_inference(
        ...
        if divisible_by(running_compress_seq_len, self.compress_block_size):
            ...
            k_compress_input = k_compress_input + self.k_intrablock_positions[None, :, None]
            # v_compress_input = einx.add('b h w n d, h n d', v_compress_input, self.v_intrablock_positions)
            v_compress_input = v_compress_input + self.v_intrablock_positions[None, :, None]
        ...
        if exists(fmask):
            ...
            fmask_expanded = fmask.unsqueeze(2).expand_as(fsim) 
            neg_sim = max_neg_value(fsim)
            fsim = torch.where(fmask_expanded, fsim, neg_sim)
    ...
    def forward(
        ...
        if has_selected_kv_for_fine_attn:
            ...
            if self.compress_block_sliding_stride != self.selection_block_size:
                ...
                if not is_empty(importance_scores):
                    ...
                    block_diagonal_mask = q_seq[:,None] == k_seq[None,:]
        ...
        if has_selected_kv_for_fine_attn:
            ...
            if self.use_triton_kernel and not disable_triton_kernel:
                ...
                if seq_len < fine_divisible_seq_len:
                    ...
                    # fk, fv = map(pad_to_multiple, (fk, fv)) # TODO
                ...
                if self.use_diff_topk:
                    ...
                    gates_expanded = gates.unsqueeze(-1).unsqueeze(-1)  # (b, h, i, sel, 1, 1)
                    gates_expanded = gates_expanded.expand_as(fk)       # (b, h, i, sel, j, d)
                    fk = gates_expanded * fk
                ...
                # TODO: fix RuntimeError: einsum(): operands do not broadcast with remapped shapes [original->remapped]: [1, 32, 1, 8, 128]->[1, 32, 1, 8, 1, 128] [1, 32, 15, 32, 128]->[1, 32, 1, 15, 32, 128]
        ...
        if exists(sliding_window_flex_mask):
            ...
            knum = sk.shape[-2]
            qnum = sq.shape[-2]
            sk, sv = sk[..., -qnum:, :], sv[..., -qnum:, :]
            # sq = torch.cat(
            #     [torch.zeros_like(sq[:,:,:1,:], device=sq.device).repeat(1,1,knum-qnum,1), 
            #     sq],dim=-2)
            sliding_window_attn_out = self.sliding_window(sq, sk, sv)

        # combine strategies

        strategy_weighted_combine = self.to_strategy_combine(ori_q)
        out = einsum(strategy_weighted_combine, stack([compressed_attn_out, fine_attn_out[:,:,:q.shape[-2]], sliding_window_attn_out]), 'b h n s, s b h n d -> b h n d')
