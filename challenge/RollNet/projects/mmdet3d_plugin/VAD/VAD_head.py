...
class VADHead(DETRHead):
    ...
    def forward(self,
        ...
        if self.motion_decoder is not None:
            ...
            # TODO
            # [M, B, D], M=A*fut_mode
            # if kwargs.get('motion_query_scalers', None) is not None:
            #     motion_query = motion_query * kwargs['motion_query_scaler']
            ego_motion_query = motion_query[:1]
            fut_mode, ld = mode_query.shape
            if kwargs.get('motion_bias', None) is not None:
                mode_query = mode_query.unsqueeze(1)
                motion_query_bias = kwargs['motion_bias'].permute(1, 0, 2)  # [M, B, D]
                max_bias_num = min(mode_query.shape[0], motion_query_bias.shape[0]) \
                                if not motion_query_bias.shape[0] == 1 else mode_query.shape[0]
                mode_query = torch.cat([mode_query[:max_bias_num] + motion_query_bias[:max_bias_num],
                                          mode_query[max_bias_num:]], dim=0)
            if kwargs.get('motion_mems', None) is not None:
                b, n, d = kwargs['motion_mems'].shape
                mode_query = mode_query.unsqueeze(0).repeat(b,1,1)
                mode_query = mode_query.reshape(b, -1, d)
                d_motion_query,_ = kwargs['motion_ca_operator'](
                    mode_query, 
                    kwargs['motion_mems'], 
                    kwargs['motion_mems'])
                h = d_motion_query + mode_query
                d_motion_query = kwargs['motion_mlp_operator'](h)
                mode_query = h + d_motion_query
                mode_query = mode_query.reshape(b, -1, ld).permute(1,0,2)
            motion_query = (motion_query[:, None, :, :] + mode_query.view(1, fut_mode, -1, ld))
            ego_motion_query = ego_motion_query[:, :, :] + mode_query.view(fut_mode, -1, ld)
            if kwargs.get('ego_motion_bias', None) is not None:
                motion_query_bias = kwargs['ego_motion_bias'].permute(1, 0, 2)  # [M, B, D]
                max_bias_num = min(ego_motion_query.shape[0], motion_query_bias.shape[0]) \
                                if not motion_query_bias.shape[0] == 1 else ego_motion_query.shape[0]
                ego_motion_query = torch.cat([ego_motion_query[:max_bias_num] + motion_query_bias[:max_bias_num],
                                          ego_motion_query[max_bias_num:]], dim=0)
            if kwargs.get('ego_motion_mems', None) is not None:
                b, n, d = kwargs['ego_motion_mems'].shape
                ego_motion_query = ego_motion_query.permute(1, 0, 2)  # [B, M, D]
                ego_motion_query = ego_motion_query.reshape(b, -1, d)
                d_motion_query,_ = kwargs['ego_motion_ca_operator'](
                    ego_motion_query, 
                    kwargs['ego_motion_mems'], 
                    kwargs['ego_motion_mems'])
                h = d_motion_query + ego_motion_query
                d_motion_query = kwargs['ego_motion_mlp_operator'](h)
                ego_motion_query = h + d_motion_query
                ego_motion_query = ego_motion_query.reshape(b, -1, ld).permute(1,0,2)
            motion_query = torch.cat([ego_motion_query[None], motion_query[1:]], dim=0).flatten(0, 1)  # [M, B, D]
