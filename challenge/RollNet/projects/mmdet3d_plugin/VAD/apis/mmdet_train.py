...
class CustomOptimizerHook(Hook):
    ...
    def after_train_iter(self, runner):
        if isinstance(runner.model.module, deepspeed.DeepSpeedEngine):
            runner.model.module.backward(runner.outputs['loss'])
            runner.model.module.step()
            return
...
def custom_train_detector(model,
    ...
    if cfg.use_ds:
        import deepspeed
        model, optimizer,*_ = deepspeed.initialize(model=model, 
                                config=cfg.strategy,
                                )
    # print(model.level_of_models[0].clip.visual.class_embedding.shape)
    # put model on gpus
    if distributed:
    ...
    # register hooks
    if cfg.get('strategy', None) and cfg.use_ds:
    ...
    if validate:
        ...
        eval_cfg['jsonfile_prefix'] = osp.join(cfg.work_dir, time.ctime().replace(' ','_').replace(':','_'))
        # eval_cfg['jsonfile_prefix'] = osp.join('val', cfg.work_dir, timestamp)
