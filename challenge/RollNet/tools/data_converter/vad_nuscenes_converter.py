...
def create_nuscenes_infos(root_path,
                          ...
                          max_sweeps=10,
                          train_text_root_path=None,
                          val_text_root_path=None,
                          ):
    ...
    train_text_labels = {}
    if train_text_root_path:
        print("loding train text labels from: ", train_text_root_path)
        import json
        with open(train_text_root_path) as f:
            train_text_labels = json.load(f)
        print("train text labels: ", len(train_text_labels))
    val_text_labels = {}
    if val_text_root_path:
        print("loding val text labels from: ", val_text_root_path)
        import json
        with open(val_text_root_path) as f:
            val_text_labels = json.load(f)
        print("val text labels: ", len(val_text_labels))
    ...
    train_nusc_infos, val_nusc_infos = _fill_trainval_infos(
        nusc, nusc_can_bus, 
        train_text_labels.keys(), val_text_labels.keys(), test, 
        max_sweeps=max_sweeps, 
        train_text_labels=train_text_labels,
        val_text_labels=val_text_labels,
        )
...
def _get_llama_QA(scene_id, scene_texts, nus_scene):
    output = {}
    scene_data = scene_texts[scene_id]['key_frames']
    # get all the frame ids in the scene in sequence
    token_list = []
    hook_id = random.sample(scene_data.keys(),1)[0]
    sample = nus_scene.get('sample', hook_id)
    token_list.append(sample['token'])
    while sample['prev'] != '':
        sample = nus_scene.get('sample', sample['prev'])
        token_list.append(sample['token'])
    token_list = token_list[::-1]
    while sample['next'] != '':
        sample = nus_scene.get('sample', sample['next'])
        token_list.append(sample['token'])
    # get all the frame id idx
    output['frame_token_list'] = token_list
    key_frame_idx = []
    for frame_id in scene_data.keys():
        sample = nus_scene.get('sample', frame_id)
        key_frame_idx.append(token_list.index(frame_id))
    output['key_frame_idx'] = key_frame_idx
    output['frames'] = {}
    for frame_id in scene_data.keys():
        sample = nus_scene.get('sample', frame_id)
        output['frames'][frame_id] = {'timestamp': sample['timestamp']}
        ...
        for k in frame_data_qa.keys():
            QA_pairs = frame_data_qa[k]
            QA_type = k
            llama_QAs = []
...
def _fill_trainval_infos(nusc,
                         ...
                         fut_ts=6,
                         his_ts=2,
                         train_text_labels=None,
                         val_text_labels=None,
                         ):
    ...
    for sample in mmcv.track_iter_progress(nusc.sample):
        ...
        if sample['scene_token'] in train_scenes:
            info['text_labels']=_get_llama_QA(sample['scene_token'], train_text_labels, nusc)
        if sample['scene_token'] in  val_scenes:
            info['text_labels']=_get_llama_QA(sample['scene_token'], val_text_labels, nusc)
        # info['query_anwser'] = text_labels[sample['token']]
...
def nuscenes_data_prep(root_path,
    ...
    create_nuscenes_infos(
        ...
        train_text_root_path=train_text_root_path,
        val_text_root_path=val_text_root_path,
        )
...
parser.add_argument('--train_text_root_path', type=str, default='v1_1_train_nus.json')
parser.add_argument('--val_text_root_path', type=str, default='v1_1_val_nus_q_only.json')
...
if __name__ == '__main__':
    if args.dataset == 'nuscenes' and args.version != 'v1.0-mini':
        ...
        nuscenes_data_prep(
            ...
            train_text_root_path=args.train_text_root_path,
            val_text_root_path=args.val_text_root_path,
            )
