...
def get_ego_fut_traj(sample, nusc, cs_record, pose_record, fut_ts):
...
def visualize_sample(nusc: NuScenes,
    ...
    # try:
    # Show prompts
    if 'textual_results' in pred_data.keys():
        print('>>>>>>QA>>>>>>')
        prompts = pred_data['textual_results'][sample_token]['prompts']
        prompts = llm_tokenizer.decode(prompts[0].int().tolist())
        decoded = pred_data['textual_results'][sample_token]['decoded'][0]
        text = f'{prompts}\n{decoded}'
        print(text)
        lines = text.split('\n')
        wrapped_lines = [textwrap.fill(line, 105) for line in lines]
        text = '\n'.join(wrapped_lines)
        plt.figure(figsize=(6.65, 3))
        plt.text(0.0, 1.0, text, fontsize=10, ha='left', va='top', color='black',
                # bbox=dict(facecolor='yellow', alpha=0.1, boxstyle='square,pad=0.5')
                )
        plt.axis('off')
        plt.tight_layout()
        plt.savefig(savepath+f'/prompt_{n_id}.png', dpi=300, bbox_inches='tight')
        # plt.show()
        plt.close()
        # except:
        #     print('No prompts')
    ...
    gt_vecs_pts_loc, gt_vecs_label = get_gt_vec_maps(
        sample_token, data_root=data_root, pc_range=map_pc_range,
        padding_value=padding_value, map_classes=map_classes,
        map_fixed_ptsnum_per_line=map_fixed_ptsnum_per_line
    )
    # print('gt_vecs_pts_loc:', dir(gt_vecs_pts_loc))
    gt_vecs_pts_loc = gt_vecs_pts_loc.fixed_num_sampled_points_torch
    for i in range(gt_vecs_pts_loc.shape[0]):
        gt_vecs_pts_loc_i = gt_vecs_pts_loc[i]
        gt_vecs_label_i = gt_vecs_label[i]
        if gt_vecs_label_i == 0:
            continue
        pts_x = np.array([pt[0] for pt in gt_vecs_pts_loc_i])
        pts_y = np.array([pt[1] for pt in gt_vecs_pts_loc_i])

        axes.plot(pts_x, pts_y, color=colors_plt[gt_vecs_label_i],linewidth=2,alpha=0.2,zorder=-1)
        axes.scatter(pts_x, pts_y, color=colors_plt[gt_vecs_label_i],s=1,alpha=0.2,zorder=-1)
    ...
    # Show GT boxes.
    for box in boxes_gt:
        box.render(axes, view=np.eye(4), colors=('grey', 'grey', 'grey'), linewidth=2, alpha=0.2)
    # Show Pred boxes.
    ...
    gt_traj = get_ego_fut_traj(
        sample_rec, nusc, cs_record, pose_record, fut_ts=6)
    ...
    axes.plot(gt_traj[:, 0, 0], gt_traj[:, 0, 1], color='g', linewidth=2, alpha=0.2)
    ...
    fig.canvas.draw()
    plt.savefig(savepath+f'/bev_pred_{n_id}.png', bbox_inches='tight', dpi=200)
    plt.savefig(savepath+f'/bev_pred_{n_id}.pdf', bbox_inches='tight', dpi=200)
    plt.close()
...
if __name__ == '__main__':
    ...
    tokenizer_path = '/media/rxin/Rex/DriveLM/Llama-2-7b/tokenizer.model'
    llm_tokenizer = Tokenizer(model_path=tokenizer_path)
    ...
    video = cv2.VideoWriter(video_path, fourcc, 10, (2933, 2252), True)
    ...
    for id in tqdm(range(len(sample_token_list))):
        ...
        render_sample_data(sample_token_list[id],
                           ...
                           out_path=out_path)
        pred_path = osp.join(out_path, f'bev_pred_{n_id}.png')
        ...
        # save img
        cv2.imwrite(osp.join(out_path, f'cam_img_{n_id}.png'), cam_img)
        size = (2133, 800)
        cam_img = cv2.resize(cam_img, size)
        vis_img = cv2.hconcat([cam_img, sample_img])
        # add text img
        if os.path.exists(osp.join(out_path, f'prompt_{n_id}.png')):
            text_img = cv2.imread(osp.join(out_path, f'prompt_{n_id}.png'))
            # plt.imshow(text_img)
            # plt.show()
            text_img = cv2.copyMakeBorder(text_img, 10, 10, 10, 10, cv2.BORDER_CONSTANT, None, value = 0)
            size = (2933, 1452)
            text_img = cv2.resize(text_img, size)
            vt_img = cv2.vconcat([vis_img, text_img])
            video.write(vt_img)
        else:
            video.write(vis_img)
