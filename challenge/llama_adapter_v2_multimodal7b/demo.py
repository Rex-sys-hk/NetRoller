...
def worker(rank, gpu_id, args, data_dict):
    ...
    model, preprocess = llama.load(args.checkpoint, llama_dir, llama_type="7B", device=device, phase="after_fintune")
    model.cpu()
    # model.half()
    model.float()
    model.eval()
    model.clip_parallel('cuda')
    # model.llama_parallel('cuda')
    ...
    for batch in tqdm(dataloader):
        ...
        results = model.generate(images, prompts, temperature=0.2, top_p=0.1, max_gen_len=1)
